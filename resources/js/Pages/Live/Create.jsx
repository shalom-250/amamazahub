...
import { Radio, Send, X, Image as ImageIcon, Upload } from 'lucide-react';
...
export default function Create() {
    ...
    const handleSubmit = (e) => {
        ...
        // Inertia handles files automatically when passed in the data object
        router.post('/live', {
            title,
            description,
            thumbnail: thumbnail
                    ...
                    <div className="p-10 space-y-8">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] ml-2 block">Broadcast Thumbnail</label>
                                <div
                                    onClick={() => fileInputRef.current?.click()}
                                    className="relative aspect-video w-full rounded-2xl overflow-hidden bg-white dark:bg-gray-900 border-2 border-dashed border-gray-800 hover:border-primary/50 transition cursor-pointer flex flex-col items-center justify-center group"
                                >
                                    {thumbnailPreview ? (
                                        <img src={thumbnailPreview} className="w-full h-full object-cover" />
                                    ) : (
                                        <div className="flex flex-col items-center space-y-3 opacity-40 group-hover:opacity-100 transition">
                                            <div className="w-16 h-16 rounded-2xl overflow-hidden opacity-50">
                                                <img src={logo} className="w-full h-full object-contain" />
                                            </div>
                                            <div className="flex items-center space-x-2">
                                                <Upload size={18} />
                                                <span className="text-[10px] font-black uppercase tracking-widest">Click to upload custom image</span>
                                            </div>
                                            <p className="text-[8px] font-bold text-gray-600 uppercase">Default: AmazamaHub Logo</p>
                                        </div>
                                    )}
                                    <input
                                        type="file"
                                    ...
                                    {thumbnailPreview && (
                                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                                            <p className="text-[10px] font-black text-white uppercase tracking-widest bg-black/60 px-4 py-2 rounded-full border border-white/10">Change Image</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-gray-900 border-2 border-transparent focus:border-primary/50 py-4 px-6 rounded-2xl text-white font-medium transition placeholder-gray-700 aspect-[5/2] resize-none"
                                />
                            </div>
